...
increment = np.uniform ( -0.25, 1.25, ( N_pop * time ) )
    ...
    for j in range ( 0, N_pop ):
        ...
        for t in range ( 1, time ): #From t = 1 to end of time
            ...
            if p_mut > np.random_sample ( ):
                n_mut [t] += 1
                g += increment [t+j] #Growth rate of subsequent mutations are stochastically/environmentally determined by an increment => intermediate levels of selection
                m_prev += m_hold
                g_total += m
                m = 1
                trans += 1
