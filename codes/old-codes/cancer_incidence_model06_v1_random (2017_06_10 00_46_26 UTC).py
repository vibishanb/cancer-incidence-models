...
g_track = [ [0 for i in range ( 0, time )] for j in range ( 0, a ) ] #Tracking values of growth rate as it changes with mutation
    ...
    for j in range ( 0, N_pop ):
        ...
        for t in range ( 1, time ): #From t = 1 to end of time
            ...
            if p_mut > np.random_sample ( ):
                n_mut [t] += 1
                g = growth_rate [j+1] #Growth rate value for subsequent mutations are random, even within an individual
                m_prev += m_hold
                g_total += m
                m = 1
                trans += 1
